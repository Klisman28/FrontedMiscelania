	...
	const columns = useMemo(() => [
		{
			...
			Cell: props => (
				<div className="flex items-center">
					<Avatar size={32} shape="circle" className="mr-3 bg-cyan-100 text-cyan-600 font-bold">
						{props.value ? props.value.charAt(0).toUpperCase() : 'C'}
					</Avatar>
		...
		},
		{
			...
			accessor: 'dni',
			sortable: true,
			Cell: props => <span className="text-slate-600 tabular-nums font-medium">{props.value}</span>
		},
		{
			Header: 'Email',
			accessor: 'email',
			sortable: true,
			Cell: props => props.value ? <a href={`mailto:${props.value}`} className="text-slate-600 hover:text-indigo-600 truncate block max-w-[180px]" title={props.value}>{props.value}</a> : <span className="text-slate-400">-</span>
		},
		{
			Header: 'Teléfono',
			accessor: 'telephone',
	...
	return (
		<div className="flex flex-col gap-4">
			...
			<div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
				<div>
					<h3 className="text-xl font-bold text-slate-900 tracking-tight">Clientes</h3>
					<p className="text-slate-500 text-sm mt-1">Gestiona tus clientes para ventas y facturación</p>
				</div>
