...
const DataTable = (props) => {
	...
	const { pageSize: _pageSize, pageIndex: _pageIndex, total: _total } = pagingData
	const uiPageSize = Number(_pageSize) || 10
	const uiPageIndex = Number(_pageIndex) || 1
	const uiTotal = Number(_total) || 0
	...
	useEffect(() => {
		const next = Math.max(uiPageIndex - 1, 0)
		if (tableState.pageIndex !== next) gotoPage(next)
		if (tableState.pageSize !== uiPageSize) setPageSize(uiPageSize)
	}, [uiPageIndex, uiPageSize, gotoPage, setPageSize, tableState.pageIndex, tableState.pageSize])
			...
			<div className="flex items-center justify-between mt-4">
				...
				<div style={{ minWidth: 130 }}>
					<Select
						...
						menuPlacement="top"
						isSearchable={false}
						value={pageSizeOption.filter(option => option.value === uiPageSize)}
